...
export function PricingStep() {
  ...
  // Initialize margins with default values
  useEffect(() => {
  ...
  const calculateTotalValue = () => {
    return products.reduce((total, product) => {
      ...
      return total + discountedPrice * product.volume;
    }, 0);
  };
  ...
  const calculateTotalMargin = () => {
    ...
    products.forEach(product => {
      const margin = margins[product.id] || 0;
      const discountedPrice = product.basePrice * (1 - margin);
      totalRevenue += discountedPrice * product.volume;
      totalCost += product.cost * product.volume;
    });
  ...
  const getProductTotal = (productId: string) => {
    const product = products.find(p => p.id === productId);
    if (!product) return 0;
    return getEffectivePrice(productId) * product.volume;
  };
                ...
                return <Fragment key={product.id}>
                      <tr className="hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => toggleProductExpansion(product.id)}>
                        ...
                        <td className="p-4">
                          ...
                          updateMargin(product.id, value / 100);
                                  ...
                                  <div className="p-4 bg-white rounded-md border border-gray-200">
                                    <div className="space-y-3">
                                      ...
                                      <div className="flex justify-between items-center">
                                        ...
                                        <span className="font-medium">
                                          {Math.round(product.volume / 12).toLocaleString()}{' '}
                                          {product.unit.split(' ')[1]}
                                        </span>
                                      </div>
                                      <div className="flex justify-between items-center">
