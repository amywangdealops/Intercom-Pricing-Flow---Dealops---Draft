...
export function VolumeStep() {
  ...
  // Handle phase change
  const handlePhaseChange = (index: number, field: string, value: number) => {
              ...
              {phases.map((phase, index) => <div key={index} className="p-4 border border-gray-200 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="number" min="1" max="36" value={phase.months} onChange={e => handlePhaseChange(index, 'months', parseInt(e.target.value) || 1)} className="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">
                        Discount (%)
                      </label>
                      <input type="number" min="0" max="100" value={phase.discount} onChange={e => handlePhaseChange(index, 'discount', parseInt(e.target.value) || 0)} className="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" />
                    </div>
                  </div>
                </div>)}
              ...
              {products.map(product => <div key={product.id}>
                  <div className="p-4 hover:bg-gray-50 cursor-pointer flex items-center justify-between" onClick={() => toggleProductExpansion(product.id)}>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      updateProductVolume(product.id, value);
